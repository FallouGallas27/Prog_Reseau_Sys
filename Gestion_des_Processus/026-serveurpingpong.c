...
int main(int argc , char*argv[]){
    ...
    //3.Appel binnd
        ...
        char bufferRecive[10]= "Ping"; int i=0;
        while(1){
        ...
        printf("[#Server# %d] (Server) %s :%d > %s:%d : %s",i ,inet_ntoa(addrServer.sin_addr),ntohs(addrServer.sin_port),inet_ntoa(addrClient.sin_addr),ntohs(addrClient.sin_port),bufferSend);
         i++;
                sleep(1);
        //recevoir port
                memset(bufferRecive,0,10);
        recv(sockClientTCP,bufferRecive,5*sizeof(char),0);
        printf("[#Server# %d ] %s:%d > %s:%d(Server) : %s\n",i ,inet_ntoa(addrClient.sin_addr),ntohs(addrClient.sin_port),inet_ntoa(addrServer.sin_addr),ntohs(addrServer.sin_port),bufferRecive);
            i++;
        }
        close(sockClientTCP);
    }
